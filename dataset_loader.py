...
class DatasetLoader(object):
  ...
  def load_from_save(self):
    ...
    print 'first: ',np.shape(self._images),np.shape(self._labels),  np.shape(self._images_test),np.shape(self._labels_test)
    ...
    print 'second: ',np.shape(self._images),np.shape(self._labels),  np.shape(self._images_test),np.shape(self._labels_test)
    
    self._labels      = self._labels.reshape([-1, len(EMOTIONS)])
    self._labels_test = self._labels_test.reshape([-1, len(EMOTIONS)])
    print 'third: ',np.shape(self._images),np.shape(self._labels),  np.shape(self._images_test),np.shape(self._labels_test)
  @property
  def images(self):
    return self._images
